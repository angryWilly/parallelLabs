...
public:
	...
	static std::string toString(uint8_t* digest);
...
void SHA256::update(std::string& s) {
	std::string str = s;
...
std::string SHA256::toString(uint8_t* digest) {
	std::stringstream s;
...
void SendInputData(int rank, std::string& input)
{
...
void GuessNonce(std::string resultString, std::string input, unsigned nonce, SHA256 sha)
{
...
int main(int argc, char** argv)
{
	...
	if (rank == 0)
	{
		std::cout << "Difficulty : " << K << std::endl;
		std::cout << "Enter a string: ";
		std::cin >> input;
	}

	SendInputData(rank, input);

	MPI_Irecv(0, 0, MPI_INT, 0, 1, MPI_COMM_WORLD, &request);

	unsigned int nonce = INT_MAX / (rank + 1);
	...
	while (rank != 0)
	{
		...
		GuessNonce(resultString, input, nonce, sha);
	...
	if (rank == 0)
	{
		...
		while (!found)
		{
			...
			std::string hash = sha.hash(resultString);
			found = IsValid(hash);

			if (found)
			{
				std::cout << "Operation took : " << MPI_Wtime() - startTime << "s" << std::endl;
				for (int i = 1; i < size; ++i)
					MPI_Send(0, 0, MPI_INT, i, 1, MPI_COMM_WORLD);

				std::cout << "Found hash(" << input << " + " << nonce << ") = " << hash << std::endl;
			}
		}
	}
